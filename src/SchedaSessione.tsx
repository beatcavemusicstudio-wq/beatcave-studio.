 ...
 */
 ...
 function formatDataBreve(iso: string): string {
   ...
   return `${gg.charAt(0).toUpperCase() + gg.slice(1)} ${d.getDate()} ${MESI[d.getMonth()]}`;
 }
         ...
         <div>
           ...
           <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
             {TIPI_SESSIONE.map(({ tipo: t, colore }) => {
               const sel = tipo === t;
               return <button key={t} onClick={() => setTipo(t)} style={{ background: sel ? `${colore}18` : "#fff", border: `${sel ? "1.5px" : "0.5px"} solid ${sel ? colore : C.border}`, borderRadius: 10, padding: "11px 12px", display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}><div style={{ width: 10, height: 10, borderRadius: "50%", background: colore, flexShrink: 0 }} /><span style={{ fontSize: 13, fontWeight: sel ? 700 : 500, color: sel ? colore : "#555" }}>{t}</span></button>;
             })}
           </div>
         </div>
 ...
 export default function SchedaSessione({ sessione, onClose, onAggiorna, onElimina, onDuplica }: Props) {
   ...
   const badge  = badgeStato(dati.stato);
   const colore = SESSION_COLORS[dati.tipo];
       ...
       <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", fontFamily: "'SF Pro Text','Helvetica Neue',Arial,sans-serif", WebkitFontSmoothing: "antialiased" }}>
         ...
         <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12, paddingBottom: 24 }}>
           ...
           <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 4 }}>
             <button onClick={() => setModificaMode(true)} style={{ width: "100%", padding: "14px", borderRadius: 12, border: "none", background: C.orange, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>Modifica sessione</button>
