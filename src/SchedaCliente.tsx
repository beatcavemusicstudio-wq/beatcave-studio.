 ...
 * Fix: Dynamic Island padding + layout compatto
 */
...
function FormCliente({ cliente, onSalva, onClose }: { cliente?: Cliente; onSalva: (c: Omit<Cliente, "id" | "dataCreazioneISO">) => void; onClose: () => void; }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 430, margin: "0 auto", background: "#fff", borderRadius: "16px 16px 0 0", padding: "20px 16px 32px" }}>
        <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 16 }}>{cliente ? "Modifica cliente" : "Nuovo cliente"}</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button onClick={() => { if (valida()) onSalva({ nome: nome.trim(), email: email.trim(), telefono: tel.trim() }); }}
            ...
            {cliente ? "Salva modifiche" : "Aggiungi cliente"}
          </button>
        </div>
      </div>
    </div>
...
// ─────────────────────────────────────────────────────────
// SCHEDA SINGOLO CLIENTE
// ─────────────────────────────────────────────────────────

function SchedaCliente({ cliente, clienteIndex, sessioni, onClose, onModifica, onNuovaPrenotazione }: {
  cliente: Cliente; clienteIndex: number; sessioni: SessioneCompleta[];
  onClose: () => void; onModifica: (c: Cliente) => void; onNuovaPrenotazione: (id: number) => void;
}) {
  ...
  return (
    <>
      ...
      {showModifica && <FormCliente cliente={cliente} onSalva={handleModifica} onClose={() => setShowModifica(false)} />}
      ...
      <div style={{ width: "100%", minHeight: "100dvh", background: C.bg, display: "flex", flexDirection: "column", fontFamily: "'SF Pro Text','Helvetica Neue',Arial,sans-serif", WebkitFontSmoothing: "antialiased" }}>
        ...
        <div style={{
          ...
          <Avatar nome={cliente.nome} color={colore} size={52} />
          <div style={{ fontSize: 22, fontWeight: 700, color: "#fff", marginTop: 10, letterSpacing: "-0.3px" }}>{cliente.nome}</div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginTop: 3 }}>
            {cliente.email}{cliente.telefono ? ` · ${cliente.telefono}` : ""}
          </div>

          {/* Strip stats */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 1, background: "rgba(255,255,255,0.08)", borderRadius: 10, overflow: "hidden", marginTop: 14 }}>
            {[
              ...
              <div key={i} style={{ background: "rgba(255,255,255,0.05)", padding: "10px 8px", textAlign: "center" }}>
                <div style={{ fontSize: 17, fontWeight: 700, color: item.color, lineHeight: 1 }}>{item.num}</div>
        ...
        {/* BODY */}
        <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12, paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 80px)" }}>

          {/* CONTATTI */}
          <div style={{ background: "#fff", border: `0.5px solid ${C.border}`, borderRadius: 12, padding: 14 }}>
            <SectionLabel>Contatti</SectionLabel>
            {[
              { lbl: "Email",      val: cliente.email,    color: C.orange },
              { lbl: "Telefono",   val: cliente.telefono || "—", color: "#222" },
              { lbl: "Cliente dal", val: formatMeseAnno(cliente.dataCreazioneISO), color: "#222" },
            ].map((row, i, arr) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: i < arr.length - 1 ? `0.5px solid rgba(0,0,0,0.05)` : "none" }}>
                <span style={{ fontSize: 12, color: "#888" }}>{row.lbl}</span>
                <span style={{ fontSize: 12, fontWeight: 600, color: row.color, maxWidth: "60%", textAlign: "right", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row.val}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "#fff", border: `0.5px solid ${C.border}`, borderRadius: 12, padding: 14 }}>
            ...
            {sessioniOrdinate.length === 0 ? (
              ...
              sessioniOrdinate.map((s, i) => (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < sessioniOrdinate.length - 1 ? `0.5px solid rgba(0,0,0,0.05)` : "none" }}>
      ...
      <div style={{ width: "100%", minHeight: "100dvh", background: C.bg, display: "flex", flexDirection: "column", fontFamily: "'SF Pro Text','Helvetica Neue',Arial,sans-serif", WebkitFontSmoothing: "antialiased" }}>
        ...
        <div style={{
          ...
          <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 10, padding: "9px 12px", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <input type="text" placeholder="Cerca cliente…" value={ricerca} onChange={e => setRicerca(e.target.value)}
              style={{ background: "none", border: "none", outline: "none", fontSize: 13, color: "#fff", width: "100%" }} />
          </div>
        </div>

        {/* LISTA */}
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px", paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 80px)", display: "flex", flexDirection: "column", gap: 8 }}>
