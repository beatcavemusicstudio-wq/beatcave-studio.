/**
 * BEATCAVE STUDIO — App principale con Supabase
 * File: App.tsx
 */
 ...
 function LoadingScreen() {
   return (
     <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: C.dark, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'SF Pro Text','Helvetica Neue',Arial,sans-serif" }}>
 ...
 function ErrorScreen({ msg, onRetry }: { msg: string; onRetry: () => void }) {
   return (
     <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, fontFamily: "'SF Pro Text','Helvetica Neue',Arial,sans-serif" }}>
 ...
 function Topbar() {
   ...
   return (
     <div style={{ background: C.dark, padding: "12px 16px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
       <div>
         <div style={{ fontSize: 22, fontWeight: 700, color: "#fff", letterSpacing: "-0.5px", lineHeight: 1.1 }}>
           Beat<span style={{ color: C.orangeMid }}>cave</span>
         </div>
         <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 3 }}>{data}</div>
       </div>
       <div style={{ width: 36, height: 36, borderRadius: "50%", background: C.orange, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff" }}>VS</div>
     </div>
   );
 }
         ...
         {celle.map((cella, i) => {
           ...
           return (
             <div key={i} style={{ aspectRatio: "1", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, borderRadius: 6, position: "relative", background: èOggi ? C.orange : "transparent", color: èOggi ? "#fff" : cella.corrente ? "inherit" : "#ccc", fontWeight: èOggi ? 700 : 400 }}>
               {cella.giorno}
               {haSessione && <div style={{ position: "absolute", bottom: 3, left: "50%", transform: "translateX(-50%)", width: 4, height: 4, borderRadius: "50%", background: èOggi ? "rgba(255,255,255,0.8)" : C.orange }} />}
             </div>
           );
         })}
       </div>
 ...
 function SessioneCard({ sessione, onTap }: { sessione: SessioneCompleta; onTap: () => void }) {
   ...
   return (
     <div onClick={onTap} style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", border: `0.5px solid ${C.border}`, borderRadius: 10, padding: "10px 12px", cursor: "pointer" }}>
       <div style={{ width: 3, height: 38, borderRadius: 2, background: SESSION_COLORS[sessione.tipo as TipoSessione], flexShrink: 0 }} />
 ...
 function TabBar({ active, onChange }: { active: TabId; onChange: (t: TabId) => void }) {
   ...
   return (
     <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderTop: `0.5px solid ${C.border}`, background: "#fff" }}>
       {tabs.map(tab => (
         <button key={tab.id} onClick={() => onChange(tab.id)} style={{ padding: "9px 0 11px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, background: "none", border: "none", cursor: "pointer" }}>
 ...
 export default function App() {
   ...
   // Salva nuovo cliente su Supabase e aggiorna la lista locale
   ...
   const handleNuovaPrenotazione = async (p: Prenotazione) => {
     try {
       const row = await inserisciSessione({
         ...
         cliente_email: p.clienteEmail,
         data:          p.data,
         ora_inizio:    p.oraInizio,
         ora_fine:      p.oraFine,
         tipo:          p.tipo,
         stato:         "confermata",
         prezzo:        p.prezzo,
