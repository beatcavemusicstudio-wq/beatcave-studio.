 ...
 *
 * Riceve la lista clienti reale da App.tsx invece di usare dati finti.
 */
 ...
 // ✏️ Tariffe default — 0 = campo vuoto
 ...
 function ModalClienti({
   clienti, onSelect, onNuovo, onClose,
 }: {
   clienti: Cliente[];
     ...
     <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
       <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 430, margin: "0 auto", background: "#fff", borderRadius: "16px 16px 0 0", padding: "20px 16px 32px", maxHeight: "70vh", display: "flex", flexDirection: "column" }}>
         ...
         <div style={{ overflowY: "auto", flex: 1 }}>
           {filtrati.length === 0 && (
             <div style={{ fontSize: 13, color: "#aaa", textAlign: "center", padding: "16px 0" }}>Nessun cliente trovato</div>
           )}
           {filtrati.map(c => (
             <div key={c.id} onClick={() => onSelect(c)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 4px", cursor: "pointer", borderBottom: "0.5px solid rgba(0,0,0,0.06)" }}>
               <div style={{ width: 38, height: 38, borderRadius: "50%", background: C.orange, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                 {iniziali(c.nome)}
               </div>
               <div>
 ...
 function ModalNuovoCliente({
   ...
   onSalva: (c: { nome: string; email: string; telefono: string }) => void;
   onClose: () => void;
 }) {
 ...
 interface Props {
   clienti: Cliente[];                                                    // ← ricevuto da App
   onSalva: (p: Prenotazione) => void;
   onClose: () => void;
   onNuovoCliente: (c: { nome: string; email: string; telefono: string }) => Promise<Cliente>; // ← salva su Supabase e restituisce il cliente con id
 }
 
 export default function NuovaPrenotazione({ clienti, onSalva, onClose, onNuovoCliente }: Props) {
   ...
   // Aggiunge nuovo cliente via Supabase e lo seleziona
   const handleNuovoCliente = async (dati: { nome: string; email: string; telefono: string }) => {
     ...
     try {
       ...
       alert("Errore nel salvare il cliente. L'email potrebbe già esistere.");
     } finally {
       setSalvando(false);
     }
   };
   ...
   return (
     <>
       {showModalClienti && !showModalNuovo && (
         <ModalClienti
           clienti={clienti}
           onSelect={c => { setCliente(c); setShowModalClienti(false); }}
           onNuovo={() => setShowModalNuovo(true)}
           onClose={() => setShowModalClienti(false)}
         />
       )}
       {showModalNuovo && (
         <ModalNuovoCliente
           onSalva={handleNuovoCliente}
           onClose={() => setShowModalNuovo(false)}
         />
       )}
       {showSuccesso && (
         <div style={{ position: "fixed", top: 60, left: "50%", transform: "translateX(-50%)", background: C.green, color: "#fff", borderRadius: 12, padding: "12px 20px", fontSize: 14, fontWeight: 700, zIndex: 200, whiteSpace: "nowrap" }}>
           ✓ Prenotazione salvata!
         </div>
       )}
       {salvando && (
         <div style={{ position: "fixed", top: 60, left: "50%", transform: "translateX(-50%)", background: "#333", color: "#fff", borderRadius: 12, padding: "12px 20px", fontSize: 14, fontWeight: 700, zIndex: 200, whiteSpace: "nowrap" }}>
       ...
       <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", fontFamily: "'SF Pro Text','Helvetica Neue',Arial,sans-serif", WebkitFontSmoothing: "antialiased" }}>
         ...
         {/* HEADER */}
         <div style={{ background: C.dark, padding: "12px 16px 16px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
             ...
             <div onClick={() => setShowModalClienti(true)} style={{ background: "#fff", border: `0.5px solid ${errori.cliente ? C.red : C.border}`, borderRadius: 12, padding: "11px 13px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
               {clienteSelezionato ? (
                 <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                   <div style={{ width: 30, height: 30, borderRadius: "50%", background: C.orange, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#fff" }}>
                     {iniziali(clienteSelezionato.nome)}
                   </div>
                   <div>
